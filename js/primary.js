...
function checkMobile() {
    ...
    if(mobile) {
        ...
        headerDiv.style.backgroundImage = "url('/imgs/MobileBackground.png')";
    }
}
...
function removeHeaderBreaks() {
    if(window.innerWidth <= 1200) {
        var leftNodes = document.getElementById("left-header").getElementsByTagName("br");
        var rightNodes = document.getElementById("right-header").getElementsByTagName("br");

        for (let i = 2; i < leftNodes.length; i++) {
            const lineBreak = leftNodes[i];
            lineBreak.style.display = "none";
        }

        for (let i = 0; i < rightNodes.length; i++) {
            const lineBreak = rightNodes[i];
            lineBreak.style.display = "none";
        }
    }
